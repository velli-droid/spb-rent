...
export default function CatalogRoot({ config }) {
    ...
    useEffect(() => {
        calendarClient.getApartments(config.humans, config.beginDate, config.endDate).then((res) => {
            if (res.status === 200) {
                    ...
                    <>
                        <div className={`catalog__grid ${isAllShown ? 'show-all' : ''}`}>
                            {apartmentsData.map((item) => {
                                return <ApartmentCard key={item.id} item={item} setSelectedApartmentId={setSelectedApartmentId}/>
                            })}
                        </div>
                        <ApartmentModal apartmentsData={apartmentsData} selectedId={selectedApartmentId} setSelectedId={setSelectedApartmentId} isAuth={config.isAuth}/>
                    </>
                )}
                {Boolean(apartmentsData.length - 6 > 0) && (
                    <button onClick={showAllClickHandler} className="catalog__more-btn btn btn_secondary">
